...
def arr(s): return "".join(json.loads(s))
cover=Image.open(io.BytesIO(base64.b64decode(arr(m.group(1))))).convert("RGB")
bg=Image.open(io.BytesIO(base64.b64decode(arr(m.group(2))))).convert("RGB").resize(cover.size,Image.Resampling.LANCZOS)
