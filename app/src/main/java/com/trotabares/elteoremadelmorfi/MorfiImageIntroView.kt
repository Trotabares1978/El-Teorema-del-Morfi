...
class MorfiImageIntroView(context: Context) : View(context) {
    ...
    init {
        bitmap = Base64.decode(IMAGE_B64, Base64.DEFAULT).let { bytes ->
    ...
    override fun onDraw(c: Canvas) {
        ...
        if (!entered) {
            val reveal = ease(((t - .25f) / 5.0f).coerceIn(0f,1f))
            val dst = coverRect(w,h)
            c.save()
            c.clipRect(0f,0f,w,dst.bottom*reveal)
            c.drawBitmap(bitmap,null,dst,paint)
            c.restore()
            if (t > 5.25f) {
                val a=ease((t-5.25f)/.8f)
                paint.color=Color.argb((245*a).toInt(),255,255,255)
                c.drawRect(0f,h*.86f,w,h,paint)
                paint.color=Color.argb((255*a).toInt(),55,43,32)
                paint.textAlign=Paint.Align.CENTER
                paint.typeface=Typeface.create("sans-serif-condensed",Typeface.BOLD)
                paint.textSize=w*.052f
                c.drawText("EL TEOREMA DEL MORFI",w/2f,h*.91f,paint)
                paint.typeface=Typeface.DEFAULT
                paint.textSize=w*.027f
                c.drawText("Más que un libro",w/2f,h*.945f,paint)
            }
        ...
        val dst=coverRect(w*1.16f,h*1.16f)
        c.save(); c.translate((w-dst.width())/2f,-h*.07f)
        c.drawBitmap(bitmap,null,RectF(0f,0f,dst.width(),dst.height()),paint); c.restore()
        ...
        paint.textSize=w*.043f; c.drawText("EL MUNDO DE MATÍAS",w/2f,h*.75f,paint)
        paint.typeface=Typeface.DEFAULT; paint.textSize=w*.027f
        c.drawText("Elegí por dónde empezar",w/2f,h*.785f,paint)
    ...
    private fun coverRect(w:Float,h:Float):RectF {
    ...
    override fun onTouchEvent(e:MotionEvent):Boolean {
        if(e.action==MotionEvent.ACTION_UP && !entered) {
            ...
            if(t>=5.8f){ entered=true; enteredAt=System.currentTimeMillis(); postInvalidateOnAnimation() }
        }
        return true
    }
